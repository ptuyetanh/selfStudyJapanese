...
class ViewAndChoose extends Component {
    ...
    render() {
        ...
        const { grammarData } = this.props.study;
        const exampleOptionById = (id) => {
            const exampleOption = grammarData.find(value => value.grammar_id === id);
            return exampleOption ? exampleOption.example : '';
        }
        return (
            <div>
                <div className="viewAndChoose">
                    <h2>Hãy xem và chọn đáp án</h2>
                    <div className="view_grammar">{this.props.mean}</div>
                    ...
                    {this.state.showAlertStudy === 'error' && <AlertErrorStudy name={this.props.name} mean={this.props.mean} example={this.props.example} mean_example={this.props.mean_example} howToLearnBack={this.props.howToLearnBackChooseName} />}
                </div>
            </div>
        );
    }
...
export default connect(mapStateToProps)(ViewAndChoose);